...
**Latest release: v0.6.0 — RELEASED 2026-07-23.** It bundles v0.5b font-family adherence and runtime-family repair (#34/#35), v0.5c contrast/tap-target measurement repair plus `layoutMetrics` (#36), v0.5d report-copy honesty (#37), and v0.5e/v0.5f CSS-usable guide literals (#38/#39). `SCHEMA_VERSION` remains `0.2`; historical artifacts keep their producer versions. **Current owner-selected maintenance train — IMPLEMENTED in this checkout, not released:** documentation truth, contrast fail-closed handling for ancestor paint effects, criterion-bounded scoring/capped-finding cardinality, and the bounded deterministic-failure loop are complete locally. No version, publish, tag, or release action belongs to that train without separate approval; see `docs/ROADMAP.md`.
