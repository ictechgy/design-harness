...
**Latest milestone: v0.5c measurement repair — IN PROGRESS (opened 2026-07-20)** — repair-only, zero new detectors and zero new criteria. `dom-contrast-risk` discards alpha on both channels and fails CSS Color 4 closed-as-black, so a correct dark-theme page scores 2/100; `tap-target-risk` implements one of WCAG 2.5.8's exceptions. Steps 0–2 landed: Chromium colour probe, measurement tripwires (`meaningfulElementCount` 71/66 pinned — `textElements` is shared by four detectors, so narrowing it deletes unrelated checks), and the **clean-corpus gate, which is deliberately RED** until the contrast repair lands. `criteria.ts` and `SCHEMA_VERSION` are frozen for the milestone — computation accuracy never upgrades criterion strength. Remaining: pure-primitive extraction (never by ES import — `page.evaluate` ships source text), contrast repair, then tap-target *or* raw layout metrics. Full scope, pre-mortem, and acceptance: **`.omx/plans/RALPLAN_V05C_MEASUREMENT_REPAIR.md`**; v0.5b remains complete and unchanged.
