...
**Latest milestone: v0.5b font-family adherence — COMPLETE 2026-07-19** — add one explicit audit-time `--guide` path and one computed-list `unapproved-font-family` project-contract risk, with same-guide third-party subtree exceptions. The v0.5a compiler remains byte/hash/provenance stable and no-guide audits do no new work. Palette/spacing adherence, actual glyph-face detection, loops, benchmarks, auto-discovery, extra agent surfaces, later v0.5 items, and cut-list items remain unscheduled. Full scope, threat boundary, and acceptance criteria: **`docs/ROADMAP.md`** (committed, canonical).
