...
**Latest milestone: v0.5c measurement repair — COMPLETE 2026-07-21 (PR #36, unreleased)** — repair-only, zero new detectors and zero new criteria. Fixed `dom-contrast-risk` (both-channel alpha compositing + oklch/oklab→sRGB conversion + skip-on-unknown with a `contrastCoverage` block; a correct dark-theme page went from 2/100 to clean) and `tap-target-risk` (WCAG 2.5.8 Spacing exception, conjunctive predicate). Pure geometry/colour maths live in `packages/visual-audit/src/measurement-primitives.ts` (extracted to Node — never imported into the `page.evaluate` closure, which ships source text and would throw `ReferenceError`); every gate is two-sided and falsified by injection. Added measurement-only `layoutMetrics` (raw margin/padding/gap/border-radius/line-height/letter-spacing distributions per viewport, no criterion, no finding). `criteria.ts` has no diff and `SCHEMA_VERSION` stays `0.2` — computation accuracy never upgrades criterion strength, and `layoutMetrics` is optional/additive. Full scope and pre-mortems: **`.omx/plans/RALPLAN_V05C_MEASUREMENT_REPAIR.md`**. Release is a separate owner-gated action; palette/spacing *adherence*, actual glyph-face detection, and later v0.5 items remain unscheduled. Also merged on `main`, unreleased: **v0.5d** report-copy honesty (#37), **v0.5e** guide hex literals, and **v0.5f** guide token literals — the generation guide now hands models CSS-usable literals for every token group (`#RRGGBB[AA]` hex, `8px`/`0.5rem` lengths, `'Helvetica Neue', Inter, sans-serif` family lists) instead of DTCG JSON (`docs/ROADMAP.md` v0.5e/v0.5f; `.omx/plans/RALPLAN_V05{E,F}_*.md`); `sourceHash`/`SCHEMA_VERSION`/`criteria.ts`/enums unchanged across all.
