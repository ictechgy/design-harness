...
The optional summaries are stored under each viewport's existing `measurement-<viewport>.data` evidence as `typographyVariants`, `paletteDiscipline`, and `densityComplexity`. Typography, palette, and visible-element summaries report `complete` or monotone `lower-bound` coverage; a lower bound can flag only when it already exceeds the configured maximum and is never described as a pass when it does not. Complete text evidence uses `text-flow-connectivity-v1`. When text evidence is skipped, the unsafe supported-only connected-component count is discarded and `supported-flow-root-count-v1` reports only the number of distinct represented flow roots as a conservative lower bound. That bound can flag an already-proven overage; otherwise it stays silent while an incomplete-evidence notice remains. Expected unsupported text evidence does not make the audit partial.
