...
describe("criteria policy matrix", () => {
  ...
  it("keeps the design-guide spacing criterion at the project-contract risk ceiling", () => {
    const criterion = getCriterion("visual.spacing.project-contract");
    expect(criterion).toMatchObject({
      category: "visual-polish",
      sourceRefs: ["design-guide-contract"],
      sourceStrength: "project-contract",
    ...
    expect(criterion?.remediationHint).toContain("declared spacing values");
    expect(criterion?.remediationHint).toContain("selector exception");
    expect(validateCriteriaPolicy([
      { ...criterion!, resultKind: "failure" }
    ], CRITERION_SOURCES).valid).toBe(false);
