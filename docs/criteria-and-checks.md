...
Visual-metric calibration has a deliberate browserless/live split. `pnpm check:visual-metrics-calibration` validates the closed manifest, fixture hashes, frozen IDs, exact keys, pair coverage, merchant non-regression contract, and complete recursive unrelated-corpus inventory under `pnpm validate`; it never launches Chromium. `pnpm smoke:visual-metrics` requires complete evidence from the three synthetic good/bad pairs and merchant case in the browser-equipped `example-smoke` job. It then runs all 48 unrelated recursively discovered HTML fixtures three times and compares their pinned raw-fixture and projected-result hashes so a detector change cannot silently move the corpus.

That broader corpus is a drift gate, not a completeness claim. Its projection hash covers only the three new measurement summaries, findings, and notices; unrelated pre-existing findings and notices remain allowed and outside the hash. Every corpus audit must nevertheless return successful status with an empty `failedChecks`. The manifest permits exactly one reviewed visual-metric lower-bound result: `color-adherence-incomplete.html` must retain zero visual-metric risks and one `palette-discipline-slots-skipped` notice whose skipped reason is exactly `unsupported-color: 1`. Any other visual-metric notice or risk, coverage/status or failed-check change, hash mismatch, or repeat-to-repeat drift fails. Atomic/merchant artifacts live at `runs/visual-metrics/<case-id>/`; corpus repeats live at `runs/visual-metrics/corpus/repeat-{1,2,3}/<entry-id>/`. CI uploads the root directory. Keeping the browser smoke out of `release:check` preserves the browserless release gate without weakening the live calibration.
