...
`CriterionSource` gains an optional `clausesByCriterion: Record<criterionId, string[]>`. The `wcag-2-2` source entry now maps every criterion that cites it to its WCAG 2.2 success-criterion ids (for example `a11y.language.page-lang` → `3.1.1`). The map lives on the **source**, not the criterion, so a future standard remap (WCAG 3.0, KWCAG 2.2) is a new source entry with its own mapping — mechanical, no criterion edits. `check:criteria-policy` enforces that a clause-mapped source maps every citing criterion and that WCAG ids are well-formed (new in this ADR: generalizing completeness to any clause-mapped source and the id format check; the wcag-2-2 completeness requirement itself is pre-settled by ROADMAP item 1). KWCAG 2.2 clause mapping is deferred to the Korean market slice, now tracked as v0.7 — UNSCHEDULED; WCAG 3.0 stays watch-only until Candidate Recommendation (projected ≥ Q4 2027).

### 6. Scoring weights and criterion-bounded influence

`needs-review` findings are score-exempt. Deductions are weighted by evidence tier: deterministic `failure` 1.0, deterministic `risk` 0.6, heuristic `risk` 0.25, `needs-review` 0. The v0.4a producer recorded the occurrence-additive formula as `epistemic-weight-v1` in `advisoryScore.formulaVersion`.

Post-v0.6.0 amendment (2026-07-23): the current producer emits `epistemic-criterion-max-v2`. It groups scored findings by `criterionId`, using a distinct `checkName` namespace only for legacy findings, and charges the strongest weighted occurrence in each group once. Equal text across the two namespaces remains separate. Equal-point representatives and all group members use locale-independent UTF-16 code-unit ordering. Deductions expose the representative `findingId`, complete `findingIds`, and unique `viewports`; the score exposes the pre-floor `totalDeduction` and whether it `saturated` above 100. Distinct criteria remain additive, and `needs-review` findings are absent from deduction membership.

The audit-result schema remains `0.2` and uses the formula version as a discriminator: historical v1 artifacts validate only in their original shape, while v2 requires the grouped fields. The current producer emits only v2; v1 artifacts are not rewritten, and scores from the two formula versions are not directly comparable. The advisory score remains advisory — never an objective design-quality grade.
