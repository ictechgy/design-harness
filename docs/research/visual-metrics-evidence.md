...
|---|---|---|
| `typography.variant-count.budget` — `rendered-typography-variants-v1` counts normalized computed family-stack + size + weight + style tuples | Ivory, Sinha & Hearst, CHI 2001 measured font faces, sizes, bold text, and italic text separately and reported category-dependent differences. That evidence motivates bounded review, but it does not equate those measures with the current computed tuple or validate a universal variant count, modular scale, font pairing rule, or glyph-resolved face inference. <https://flamenco.berkeley.edu/papers/chi2001.pdf> | Complete, opt-in, not released |
...
The versioned chroma cutoff, hue span, candidate caps, clipping topology, and text-fragment adjacency coefficients are reproducibility and safety constants. The six atomic fixtures and merchant case require complete exact evidence. A portable committed hash plus full same-environment repeat gate covers 48 unrelated recursive HTML fixtures and pins one reviewed lower-bound color-space notice; only font-environment-sensitive density fragment and edge-test diagnostic counts are excluded from the portable hash. This calibration gates drift and demonstrates repeatability on the pinned corpus; it neither claims complete evidence for every corpus page nor validates the constants as taste boundaries.
